...
import { SellerOrderService } from '../../../services/seller-order.service'; // updated service import
...
export class OrderListComponent implements OnInit, OnChanges {
  ...
  fetchOrders(): void {
    ...
    this.sellerOrderService
      .getSellerOrders({
        ...
        limit: this.limit,
        offset,
        order_status: this.status !== 'all' ? this.status : undefined,
  ...
  onEndDateChange(event: Event): void {
    this.endDate = (event.target as HTMLInputElement).value || undefined;
    this.currentPage = 1;
    this.fetchOrders();
  }
